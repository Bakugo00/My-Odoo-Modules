...
odoo.define('2m_production.section_and_note_backend', function (require) {
    ...
    var SectionAndNoteListRenderer = ListRenderer.extend({
        ...
        _renderBodyCell: function (record, node, index, options) {
            ...
            var isComposed_product = record.data.display_type === 'line_composed_product'
            ...
            if (isSection || isNote) {
                ...
                if (model == "account.move.line"){
                    if (node.attrs.widget === "handle") {
                        ...
                        $cell.attr('colspan', 3);
                    } else if (node.attrs.name === "new_quantity") {
                        var nbrColumns = this._getNumberOfCols();
                        if (this.handleField) {
                            nbrColumns--;
                        }
                        ...
                        // $cell.text(record.data.new_quantity)
                        $cell.attr('colspan', 2);
                    }  else if (node.attrs.name === "product_uom_id") {
                        var nbrColumns = this._getNumberOfCols();
                        if (this.handleField) {
                            nbrColumns--;
                        }
                        ...
                        $cell.text(' ')
                        $cell.attr('colspan',1);
                    } else if (node.attrs.name === "price_unit") {
                        var nbrColumns = this._getNumberOfCols();
                        if (this.handleField) {
                            nbrColumns--;
                        }
                        ...
                        $cell.text(record.data.new_price_unit)
                    $cell.attr('colspan', 1);
                } else if (node.attrs.name === "tax_ids") {
                    var nbrColumns = this._getNumberOfCols();
                        if (this.handleField) {
                            nbrColumns--;
                        }
                ...
                if (node.attrs.widget === "handle") {
                    ...
                    $cell.attr('colspan', 3);
                } else if (node.attrs.name === "product_uom_qty") {
                    var nbrColumns = this._getNumberOfCols();
                    if (this.handleField) {
                        nbrColumns--;
                    }
                    ...
                    $cell.text(record.data.new_quantité)
                    $cell.attr('colspan', 1);
                } else if (node.attrs.name === "product_uom") {
                    var nbrColumns = this._getNumberOfCols();
                    if (this.handleField) {
                        nbrColumns--;
                    }
                    ...
                    $cell.text(record.data.new_price_unit)
                    $cell.attr('colspan', 1);
                } else if (node.attrs.name === "tax_id") {
                    var nbrColumns = this._getNumberOfCols();
                    if (this.handleField) {
                        nbrColumns--;
                    }
                    ...
                    $cell.attr('colspan', nbrColumns - 7);
                } else {
                    $cell.removeClass('o_invisible_modifier');
                    return $cell.addClass('o_hidden');
                }}
    ...
    var SectionAndNoteFieldText = function (parent, name, record, options) {
        var isSection = record.data.display_type === 'line_section';
        var isComposed_product = record.data.display_type === 'line_composed_product'
        var Constructor = isSection || isComposed_product ? FieldChar : ListFieldText;
